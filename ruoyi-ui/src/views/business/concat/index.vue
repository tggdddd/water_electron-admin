    ...
    <el-table v-loading="loading" :data="chargeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" align="center" prop="dictType">
        <template #default="slot">
          {{slot.row.dictType === "qq_concat"?"QQ":"微信"}}
        </template>
      </el-table-column>
      <el-table-column label="值" align="center" prop="dictValue">
        <template #default="slot">
          <div v-if="slot.row.dictType === 'qq_concat'">{{slot.row.dictValue}}</div>
          <image-preview :width="64" :height="64" v-else :src="slot.row.dictValue"/>
        </template>
      </el-table-column>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="客服类型" prop="type">
          <el-select v-model="form.type">
            <el-option value="qq_concat" label="QQ"></el-option>
            <el-option value="wx_concat" label="微信"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="值" prop="value">
          <el-input v-if="form.type==='qq_concat'" v-model="form.value" placeholder="请输入qq号码"/>
          <image-upload v-else  :limit="1" v-model="form.value"/>
        </el-form-item>
...
<script>
import {listConcat, delConcat, addConcat, updateConcat, getConcat} from "@/api/business/concat";
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateConcat(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addConcat(this.form).then(response => {
