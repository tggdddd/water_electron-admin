  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-divider content-position="center">用户到期时间信息</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleAddUserDuration">添加</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="handleDeleteUserDuration">删除</el-button>
          </el-col>
        </el-row>
        <el-table :data="userDurationList" :row-class-name="rowUserDurationIndex" @selection-change="handleUserDurationSelectionChange" ref="userDuration">
          <el-table-column type="selection" width="50" align="center" />
          <el-table-column label="序号" align="center" prop="index" width="50"/>
          <el-table-column label="到期时间" prop="expireTime" width="240">
            <template slot-scope="scope">
              <el-date-picker clearable v-model="scope.row.expireTime" type="date" value-format="yyyy-MM-dd" placeholder="请选择到期时间" />
            </template>
          </el-table-column>
        </el-table>
      </el-form>
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        ...
        this.userDurationList = response.data.userDurationList;
        this.open = true;
        this.title = "修改用户信息";
      });
    },
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.form.userDurationList = this.userDurationList;
	...
	/** 用户到期时间序号 */
    rowUserDurationIndex({ row, rowIndex }) {
      row.index = rowIndex + 1;
    },
    /** 用户到期时间添加按钮操作 */
    handleAddUserDuration() {
      let obj = {};
      obj.expireTime = "";
      this.userDurationList.push(obj);
    },
    /** 用户到期时间删除按钮操作 */
    handleDeleteUserDuration() {
      if (this.checkedUserDuration.length == 0) {
        this.$modal.msgError("请先选择要删除的用户到期时间数据");
      } else {
        const userDurationList = this.userDurationList;
        const checkedUserDuration = this.checkedUserDuration;
        this.userDurationList = userDurationList.filter(function(item) {
          return checkedUserDuration.indexOf(item.index) == -1
        });
      }
    },
    /** 复选框选中数据 */
    handleUserDurationSelectionChange(selection) {
      this.checkedUserDuration = selection.map(item => item.index)
    },
    /** 导出按钮操作 */
    handleExport() {
